...
* **Topological Continuity Preservation (Extending *SCOPE*, MICCAI 2023):** Dr. Farshad established that biological structures require explicit graph-based topological continuity constraints (`clDice`, Betti numbers). We translate these continuity regularizers to high-DOF articulated skeletal trees to guarantee bone-length invariance ($\mathcal{L}_{\text{graph}}$) and biological strain limits ($P_{\text{strain}}$), eliminating joint popping and rubber fingers.
* **Inference-Time Scene Graph & Bounding Guidance (Extending *SceneGenie*, ICCV 2023):** Formulating 3D spatial discourse as an evolving scene graph $\mathcal{G} = (\mathcal{V}, \mathcal{E})$, we adapt SceneGenie's training-free augmented bounding-box guidance ($\nabla \mathcal{L}_{\text{aug,boxg}}$) to steer reverse diffusion sampling toward 3D anatomical contact loci ($R$-loci). In reverse, 3D semantic scene graphs ground physical room objects to disambiguate pointing gestures.
* **Hierarchy-Aware Coarse-to-Fine Diffusion (Extending *HieraSurg*, 2025):** Translating Dr. Farshad's two-stage hierarchical diffusion framework (where high-level procedural states synthesize coarse structural maps that condition continuous video), our Dual-Rate Streaming Architecture translates discrete phonological macro-tokens ($\mathcal{S}$) into coarse kinematic targets before continuous 60 Hz residual motion diffusion.
